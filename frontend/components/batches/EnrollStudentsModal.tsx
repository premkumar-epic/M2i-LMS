...
import { listUsers, enrollStudents } from "@/lib/batch.api";
...
export default function EnrollStudentsModal({ batchId, onSuccess, onClose }: Props) {
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md">
        <div className="p-5 border-b border-gray-100">
          <h2 className="font-semibold text-gray-900">Enroll Students</h2>
          <p className="text-xs text-gray-500 mt-0.5">Select students to add to this batch.</p>
        </div>
        ...
        <div className="p-5 border-t border-gray-100 flex gap-3">
          ...
          >
            {submitting ? "Enrolling..." : `Enroll ${selected.size > 0 ? selected.size : ""}`}
          </button>
        </div>
      </div>
    </div>
